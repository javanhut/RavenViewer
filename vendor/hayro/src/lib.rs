...
pub struct RenderSettings {
    ...
    /// How far to shift the contents left, in scaled pixels. Together with
    /// `width`/`height` this selects a sub-rectangle of the page, so a viewer
    /// can rasterize only the tiles it is about to show instead of the whole
    /// page at that scale.
    pub x_offset: f32,
